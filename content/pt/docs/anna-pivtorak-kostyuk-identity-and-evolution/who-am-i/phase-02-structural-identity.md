---
title: Fase II — Identidade Estrutural (2026)
description: Segunda fase da identidade – transição da narrativa para o pensamento sistémico e construção estrutural.
weight: 4
event_date: 2026-02-26T09:00:00
publication_date: 2026-02-26T09:00:00
event_type: life_node
lastmod: 2026-02-26T09:00:00
---

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Article",
  "headline": "Quem sou eu? — Fase II — Identidade Estrutural (2026)",
  "description": "Segunda fase da identidade: transição da narrativa para o pensamento sistémico e construção estrutural.",
  "author": {
    "@type": "Person",
    "name": "Anna Pivtorak",
    "alternateName": "Anna Kostyuk"
  },
  "datePublished": "2026-04",
  "temporalCoverage": "2025-03-12/2026-02-26",
  "inLanguage": "pt",
  "isPartOf": {
    "@type": "CreativeWork",
    "name": "Identity & Evolution",
    "url": "https://pivtorak.studio/pt/docs/anna-pivtorak-kostyuk-identity-and-evolution/"
  },
  "about": [
    "evolução da identidade",
    "pensamento sistémico",
    "identidade estrutural"
  ]
}
</script>

<div class="float-right">

![Quem sou eu? — Fase II — Identidade Estrutural (2026)](/images/anna-pivtorak-kostyuk-identity-and-evolution-who-am-i-phase-02-іtructural-identity.webp)

</div>

# 🌐 Anna Pivtorak (Kostyuk) – Após a formação da Pivtorak.Studio como um sistema estruturado   

## Who am I? — Phase II — Structural Identity (2026)   

Já não me defino através de papéis.  

Os papéis mudam.  
Os sistemas permanecem.  

O que faço não é uma profissão.    
É uma forma de pensar.  

Observo padrões, construo estruturas e testo-as na realidade.  
Se uma estrutura funciona — permanece.  
Se não funciona — é removida sem apego.  

O meu trabalho não é storytelling visual.  
É a construção de significado através da forma.  

Cada objeto que crio não é um resultado isolado.  
É um fragmento de um sistema maior.   

Este sistema baseia-se em:   
- análise    
- precisão    
- repetibilidade    
- e tempo    

Não dependo de inspiração.    
Dependo de clareza.  
 
Não procuro expressar.    
Procuro construir.  

O que importa não é a aparência.    
O que importa é se se sustenta.  

Se se sustenta — torna-se parte do sistema.    
Se não — desaparece.  

Não estou a construir uma coleção de obras.    
Estou a construir um sistema de pensamento que pode existir para além de mim.  

## 🧭 Código de Conduta  
(síntese: Pai + Mãe → Eu)  
### Sistema → Forma → Ação    
- Ajo através da compreensão, não da pressão  
- Mantenho a forma com a mesma precisão que o conteúdo  
- Tomo decisões de forma independente e levo-as até ao fim  
- Crio ambientes que sustentam o desenvolvimento  
- Escolho qualidade em vez de exibição  
- Penso de forma sistémica, mas ajo de forma concreta  
- Não permito caos — nem no pensamento nem na forma  
- Olho para além da situação imediata  
- Estou sempre pronta para o encontro  
- Sou responsável não só pelas ações, mas pelo meu estado  
- Trabalho com a realidade, não com suposições  
- Aperfeiçoo até à precisão — mesmo nos detalhes  

*Anna Pivtorak (Kostyuk) – Após a formação da Pivtorak.Studio como um sistema estruturado. Quem sou eu? — Fase II — Identidade Estrutural (2026). Pivtorak.Studio. 26.02.2026*
